...
/**
 * SshService — يدير اتصالات SSH لكل أجهزة الـ Cyber-Range.
 *
 * FIX 2026: قراءة stdout/stderr بشكل صحيح من JSch.
 * المشكلة السابقة: كنا نقرأ stream قبل ما يخلص الـ command،
 * النتيجة "no output" حتى لما الأمر ينجح.
 *
 * الحل: نقرأ من InputStream مباشرة في loop حتى -1 (EOF).
 */
@Service
public class SshService {
    ...
    private synchronized Session getSession(String deviceName) throws JSchException {
        ...
        if (existing != null) {
            try { existing.disconnect(); } catch (Exception e) { /* ignore */ }
        }
    ...
    /**
     * نفّذ command وأرجع المخرج.
     *
     * FIX: نقرأ stdout/stderr من InputStream مباشرة في loop،
     * بدل ما نستخدم setOutputStream (الذي قد لا يكون مكتوباً عند channel.isClosed()).
     */
