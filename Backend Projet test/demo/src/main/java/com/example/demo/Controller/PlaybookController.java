...
/**
 * PlaybookController — CRUD + test-run for mitigation playbooks.
 *
 *  GET    /api/playbooks
 *  POST   /api/playbooks              (ADMIN)
 *  PUT    /api/playbooks/{id}         (ADMIN)
 *  DELETE /api/playbooks/{id}         (ADMIN)
 *  POST   /api/playbooks/{id}/run/{sessionId}  (ADMIN) — manual test execution
 */
@RestController
...
public class PlaybookController {
    ...
    // Manual test execution against a real session (used by Security "Mitigate" button).
    // Returns JSON so the frontend's res.json() parser succeeds.
